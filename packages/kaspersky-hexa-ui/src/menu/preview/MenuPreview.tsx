import { Text } from '@src/typography'
import React, { ReactNode, useEffect, useState } from 'react'
import styled from 'styled-components'

import { Bell as AlarmIcon, BookmarkOutline, Help, Moon } from '@kaspersky/hexa-ui-icons/16'

import { getStatusIcon } from '../NavUserItem'
import { Hamburger, Menu as HexaMenu } from '../Menu'
import { ServicesNav } from '../ServicesNav'
import { MenuProps, NavItemData } from '../types'

const noop = (): void => undefined

export const DEFAULT_MENU_PREVIEW_TITLE = 'Configuration Service'
export const DEFAULT_MENU_PREVIEW_DESCRIPTION = 'Kaspersky Next'

export type MenuPreviewHeaderProps = {
  className?: string,
  logo?: ReactNode,
  title?: ReactNode,
  description?: ReactNode
}

export type PreviewUserAvailabilityHandlers = {
  onAvailable?: VoidFunction,
  onUnavailable?: VoidFunction
}

export type MenuPreviewShellProps = Omit<MenuProps, 'collapsed' | 'children'> & {
  children?: ReactNode,
  minimized?: boolean,
  showHeader?: boolean,
  showServicesNav?: boolean,
  notificationsActive?: boolean,
  headerContent?: ReactNode,
  headerLogo?: ReactNode,
  headerTitle?: ReactNode,
  headerDescription?: ReactNode,
  onNotificationClick?: VoidFunction,
  onThemeClick?: VoidFunction,
  onHelpClick?: VoidFunction
}

export const DefaultMenuPreviewLogo = (): JSX.Element => (
  <svg width="64" height="64" viewBox="0 0 128 128" fill="none" xmlns="http://www.w3.org/2000/svg">
    <g clipPath="url(#menuPreviewLogoClip0)">
      <g clipPath="url(#menuPreviewLogoClip1)">
        <path fillRule="evenodd" clipRule="evenodd" d="M113.495 33.4495L67.4951 6.93934C65.3318 5.69261 62.6682 5.69261 60.5049 6.93934L14.5049 33.4495C12.3363 34.6993 11 37.0119 11 39.5149V88.4929C11 90.9959 12.3363 93.3085 14.5049 94.5583L60.5049 121.068C62.6682 122.315 65.3318 122.315 67.4951 121.068L113.495 94.5583C115.664 93.3085 117 90.9959 117 88.4929V39.5149C117 37.0119 115.664 34.6993 113.495 33.4495ZM70.4909 1.74043C66.4733 -0.574934 61.5267 -0.574935 57.5091 1.74043L11.5091 28.2506C7.48177 30.5716 5 34.8664 5 39.5149V88.4929C5 93.1414 7.48177 97.4362 11.5091 99.7572L57.5091 126.267C61.5267 128.583 66.4733 128.583 70.4909 126.267L116.491 99.7572C120.518 97.4362 123 93.1414 123 88.4929V39.5149C123 34.8664 120.518 30.5716 116.491 28.2506L70.4909 1.74043Z" fill="var(--menu_v1--logo_brand)"/>
      </g>
      <path fillRule="evenodd" clipRule="evenodd" d="M58.0798 46.5126C58.0798 47.6124 57.1919 48.5058 56.0921 48.5126L54.8398 48.5203C54.5798 49.8003 54.0598 51.0003 53.3598 52.0603L54.2456 52.9461C55.0266 53.7271 55.0266 54.9935 54.2456 55.7745L54.214 55.8061C53.433 56.5871 52.1666 56.5871 51.3856 55.8061L50.4998 54.9203C49.4398 55.6203 48.2398 56.1403 46.9598 56.4003V57.6203C46.9598 58.7249 46.0644 59.6203 44.9598 59.6203H44.8998C43.7952 59.6203 42.8998 58.7249 42.8998 57.6203V56.3803C41.6198 56.1203 40.4198 55.6003 39.3598 54.9003L38.474 55.7861C37.6929 56.5672 36.4266 56.5671 35.6456 55.7861L35.614 55.7545C34.833 54.9735 34.833 53.7071 35.614 52.9261L36.4998 52.0403C35.7998 50.9803 35.2798 49.7803 35.0198 48.5003H33.7798C32.6752 48.5003 31.7798 47.6049 31.7798 46.5003V46.4403C31.7798 45.3357 32.6752 44.4403 33.7798 44.4403H35.0198C35.2798 43.1603 35.7998 41.9603 36.4998 40.9003L35.614 40.0145C34.833 39.2335 34.833 37.9671 35.614 37.1861L35.6456 37.1545C36.4266 36.3735 37.6929 36.3735 38.474 37.1545L39.3598 38.0403C40.4198 37.3403 41.6198 36.8203 42.8998 36.5603V35.3203C42.8998 34.2157 43.7952 33.3203 44.8998 33.3203H44.9598C46.0644 33.3203 46.9598 34.2157 46.9598 35.3203V36.5603C48.2398 36.8203 49.4398 37.3403 50.4998 38.0403L51.3856 37.1545C52.1666 36.3735 53.433 36.3735 54.214 37.1545L54.2456 37.1861C55.0266 37.9671 55.0266 39.2335 54.2456 40.0145L53.3598 40.9003C54.0598 41.9603 54.5798 43.1603 54.8398 44.4403H56.0798C57.1844 44.4403 58.0798 45.3357 58.0798 46.4403V46.5126ZM39.8398 46.4803C39.8398 49.2803 42.1198 51.5403 44.9198 51.5403C47.7198 51.5403 49.9798 49.2803 49.9798 46.4803C49.9798 43.7003 47.7198 41.4203 44.9198 41.4203C42.0998 41.4203 39.8398 43.6803 39.8398 46.4803Z" fill="var(--menu--bg--logo_icon--enabled)"/>
      <path fillRule="evenodd" clipRule="evenodd" d="M94.6983 63.9758C95.1846 64.971 94.7695 66.1719 93.7725 66.6543L90.6198 68.1798C91.1798 70.3398 91.2798 72.5398 90.9998 74.6798L94.3124 75.8264C95.3552 76.1874 95.9086 77.3248 95.5491 78.368L94.6123 81.0863C94.2521 82.1316 93.1121 82.6863 92.0672 82.3246L88.7598 81.1798C87.6598 83.0398 86.2198 84.6998 84.4598 86.0598L85.9991 89.202C86.4854 90.1946 86.0742 91.3934 85.0811 91.8787L82.5009 93.1396C81.5068 93.6254 80.3073 93.2118 79.8238 92.2167L78.2998 89.0798C76.1398 89.6398 73.9398 89.7398 71.7998 89.4598L70.6531 92.7724C70.2922 93.8152 69.1548 94.3686 68.1115 94.009L65.3933 93.0723C64.348 92.7121 63.7933 91.572 64.1549 90.5272L65.2998 87.2198C63.4398 86.1198 61.7798 84.6798 60.4198 82.9198L57.2746 84.4478C56.2827 84.9297 55.0879 84.5178 54.6037 83.527L53.34 80.9408C52.8542 79.9468 53.2678 78.7472 54.2629 78.2638L57.3998 76.7398C56.8398 74.5798 56.7398 72.3798 57.0198 70.2398L53.7072 69.0931C52.6644 68.7321 52.111 67.5948 52.4706 66.5515L53.4073 63.8332C53.7675 62.7879 54.9076 62.2332 55.9524 62.5949L59.2598 63.7398C60.3598 61.8798 61.7798 60.2198 63.5598 58.8598L62.0318 55.7146C61.5499 54.7227 61.9618 53.5279 62.9526 53.0437L65.5446 51.777C66.5364 51.2924 67.7332 51.7028 68.2188 52.6941L69.7598 55.8398C71.9198 55.2798 74.1198 55.1798 76.2598 55.4598L77.4065 52.1471C77.7675 51.1044 78.9048 50.551 79.9481 50.9105L82.6663 51.8472C83.7117 52.2075 84.2664 53.3475 83.9047 54.3923L82.7598 57.6998C84.6198 58.7998 86.2798 60.2398 87.6398 61.9998L90.7635 60.476C91.7558 59.992 92.9526 60.4035 93.4373 61.3954L94.6983 63.9758ZM66.2998 76.2398C68.3798 80.4998 73.5398 82.2798 77.7998 80.1998C82.0598 78.1198 83.8398 72.9598 81.7598 68.6998C79.6798 64.4398 74.5198 62.6598 70.2598 64.7398C65.9998 66.8198 64.2198 71.9798 66.2998 76.2398Z" fill="var(--menu--bg--logo_icon--enabled)"/>
    </g>
    <defs>
      <clipPath id="menuPreviewLogoClip0">
        <rect width="128" height="128" fill="var(--menu--bg--logo_icon--enabled)"/>
      </clipPath>
      <clipPath id="menuPreviewLogoClip1">
        <rect width="128" height="128" fill="var(--menu--bg--logo_icon--enabled)"/>
      </clipPath>
    </defs>
  </svg>
)

export const MenuPreviewHeader = ({
  className,
  logo = <DefaultMenuPreviewLogo />,
  title = DEFAULT_MENU_PREVIEW_TITLE,
  description = DEFAULT_MENU_PREVIEW_DESCRIPTION
}: MenuPreviewHeaderProps): JSX.Element => (
  <StyledMenuPreviewHeader className={className}>
    {logo}
    {(title || description) && (
      <div className="logo-text">
        {title && <Text type="BTM3" className="logo-text-osmp">{title}</Text>}
        {description && <Text type="BTR4">{description}</Text>}
      </div>
    )}
  </StyledMenuPreviewHeader>
)

export const MenuPreviewNotificationButton = ({
  hasNotifications = true,
  onClick = noop
}: {
  hasNotifications?: boolean,
  onClick?: VoidFunction
}): JSX.Element => (
  <StyledNotificationButton
    className={`item js-tutorial-notifications header-notification${hasNotifications ? ' active' : ''}`}
    role="button"
    onClick={onClick}
  >
    <AlarmIcon />
  </StyledNotificationButton>
)

export const cloneNavItems = (items: NavItemData[] = []): NavItemData[] => (
  items.map((item) => ({
    ...item,
    userProps: item.userProps ? { ...item.userProps } : item.userProps,
    items: item.items ? cloneNavItems(item.items) : item.items,
    submenuItems: item.submenuItems
      ? item.submenuItems.map((submenuItem) => ({ ...submenuItem }))
      : item.submenuItems
  }))
)

export const withPreviewUserAvailabilityItems = (
  items: NavItemData[] = [],
  handlers: PreviewUserAvailabilityHandlers = {}
): NavItemData[] => {
  const nextItems = cloneNavItems(items)
  const userItem = nextItems.find((item) => item.state === 'user')

  if (!userItem) {
    return nextItems
  }

  const userSubmenuItems = userItem.items ?? []

  if (userSubmenuItems[0]?.key === 'Availability') {
    return nextItems
  }

  userItem.items = [
    {
      key: 'Availability',
      isCaption: true
    },
    {
      key: 'Available',
      icon: getStatusIcon('available', 'dark'),
      onClick: handlers.onAvailable ?? noop
    },
    {
      key: 'Unavailable',
      icon: getStatusIcon('unavailable', 'dark'),
      onClick: handlers.onUnavailable ?? noop
    },
    ...userSubmenuItems
  ]

  return nextItems
}

export const MenuPreviewShell = ({
  children,
  minimized = false,
  showHeader = true,
  showServicesNav = true,
  notificationsActive = true,
  headerContent,
  headerDescription = DEFAULT_MENU_PREVIEW_DESCRIPTION,
  headerLogo = <DefaultMenuPreviewLogo />,
  headerTitle = DEFAULT_MENU_PREVIEW_TITLE,
  onNotificationClick = noop,
  onThemeClick = noop,
  onHelpClick = noop,
  ...menuProps
}: MenuPreviewShellProps): JSX.Element => {
  const [collapsed, setCollapsed] = useState(minimized)

  useEffect(() => {
    setCollapsed(minimized)
  }, [minimized])

  return (
    <StyledMenuPreview
      {...menuProps}
      collapsed={collapsed}
      favIcon={menuProps.favIcon ?? <BookmarkOutline />}
      pinIcon={menuProps.pinIcon ?? <BookmarkOutline />}
    >
      {showServicesNav && (
        <ServicesNav>
          <Hamburger
            className="item left"
            role="button"
            name="hamburger"
            collapsed={collapsed}
            onClick={() => setCollapsed((previousState) => !previousState)}
          />
          <MenuPreviewNotificationButton hasNotifications={notificationsActive} onClick={onNotificationClick} />
          <Moon className="item" role="button" onClick={onThemeClick} />
          <Help className="item" role="button" onClick={onHelpClick} />
        </ServicesNav>
      )}
      {showHeader && (
        headerContent ?? (
          <MenuPreviewHeader
            logo={headerLogo}
            title={headerTitle}
            description={headerDescription}
          />
        )
      )}
      {children}
    </StyledMenuPreview>
  )
}

const StyledMenuPreview = styled(HexaMenu)`
  height: 100%;
  min-height: 100%;
  background: transparent;
  box-sizing: border-box;

  .ant-layout-sider-children {
    display: flex;
    flex-direction: column;
    height: 100%;
    min-height: 100%;
    max-height: none;
    padding: 16px 0;
    gap: 24px;
  }

  .uif-nav,
  .uif-nav-item-child-wrapper {
    gap: 2px;
  }

  .uif-nav.nav-scrollable {
    flex: 1 1 auto;
    min-height: 0;
  }

  .uxpin-menu-item-hover > .uif-nav-item-entry {
    background: var(--menu_item--bg--hover);
  }

  .uif-nav-item-entry {
    padding-left: 8px;
    padding-right: 8px;
  }
`

const StyledMenuPreviewHeader = styled.div`
  min-height: 64px;
  display: flex;
  gap: 8px;
  margin: 10px 16px 16px 16px;
  color: #fff;
  text-decoration: none;
  overflow: hidden;
  align-items: center;
  opacity: 1;
  position: relative;
  transition: opacity 0.1s .15s ease;

  .logo-text {
    display: flex;
    flex-direction: column;
    gap: 2px;
    min-width: 0;
  }

  .logo-text .kl6-text {
    color: var(--menu--text--logo);
  }

  .ant-layout-sider-collapsed & {
    opacity: 0;
    transition: opacity 0s 0s ease;
    width: 0;
    height: 0;
    overflow: hidden;
    position: absolute;
  }
`

const StyledNotificationButton = styled.span`
  position: relative;
  display: inline-block;
  height: 16px;

  &.active {
    &:before {
      content: '';
      display: block;
      position: absolute;
      left: 14px;
      top: -6px;
      width: 8px;
      height: 8px;
      border-radius: 50%;
      background: var(--indicator--icon--critical);
    }
  }
`
